    ...
    For p = 1 To TotalCats
        If (Cells(i, 4).Value <> 1) And (Workers.WorkersTree.Nodes(p).Key = CStr(Cells(i, 6).Value) & "z") Then
            If (SelectUpdatesOnly And Cells(i, 1).Value = 1) Or Not SelectUpdatesOnly Then
                ...
                If AppMode = "client" Then Workers.WorkersTree.Nodes(Workers.WorkersTree.Nodes.Count).Tag = Cells(i, 7)
                p = TotalCats
            End If
        End If
    Next p
...
Else
    ...
    If AppMode = "client" And LastMonth_Label.Visible = False Then
...
If (ID.Value <> "") And (DetectChanges = 1) Then
    ...
    If Amount_Box.Value <> "." And Amount_Box.Value <> "-" And Amount_Box.Value <> "-." Then Cells(index, 4).Value = Amount_Box.Value
    Cells(index, 5).Value = Unit.Caption
    If Time_Box.Value <> "." And Time_Box.Value <> "-" And Time_Box.Value <> "-." Then Cells(index, 6).Value = Time_Box.Value
    
    If AppMode = "client" Then SetRandomMark
    
    If Oklad_Box = "" Or AboveOklad_Chk.Value = True Then
        If Rate_Box.Value <> "." And Rate_Box.Value <> "-" And Rate_Box.Value <> "-." Then Cells(index, 7).Value = Rate_Box.Value
    Else
...
If PrePay_Box.Value <> "." And PrePay_Box.Value <> "-" And PrePay_Box.Value <> "-." Then Cells(index - Job + 1, 11).Value = PrePay_Box.Value
If Left_Box.Value <> "." And Left_Box.Value <> "-" And Left_Box.Value <> "-." Then Cells(2, 10).Value = Left_Box.Value
If Oklad_Box.Value <> "." And Oklad_Box.Value <> "-" And Oklad_Box.Value <> "-." Then Cells(4, 2).Value = Oklad_Box.Value
...
MakeShitLookGood
TransferBalanceToNextMonth NameChooser.Value, Balance_Box.Value
DetectChanges = 0
FillControlBox
...
If AppMode = "server" Then Cells(index, 3) = 4
Cells(index, 2).Select
...
For i = 0 To Lines - 1
    ...
    If (AppMode = "server") And (Cells(index + i, 3) <> "") Then Cells(index + i, 3) = 4
    'If OldId > 5 Then
    'Sheets("Каталог").Select
...
'On Error GoTo ExceptionControl:
ObjectsRecall
If Amount_Box.Value <> "" And Amount_Box.Value <> 0 And Amount_Box.Value <> "-" And Amount_Box.Value <> "." And Amount_Box.Value <> "-." Then
...
If NameChooser.Value <> "" And MateChooser.Value <> "" Then
    ...
    If Cells(3, 1).Value = "RO" And AppMode = "client" Then
        Sheets(NameChooser.Value).Select
    ...
    If AppMode = "client" Then SetRandomMark
    MakeShitLookGood
    TransferBalanceToNextMonth MateChooser.Value, Cells(1, 10).Value
Sheets(NameChooser.Value).Select
End If
...
If NameChooser.Value <> "" Then
    ...
    If NameChooser.Value = MateChooser.Value Or AppMode = "client" Then
        MateChooser.Value = ""
        MateName_Box.Value = ""
    End If
End If
...
If WorkersTree.SelectedItem.Key <> "" And WorkersTree.SelectedItem.Tag <> "Cat" Then
     ...
     If WeChooseMate = 0 Then
        ...
        If (BlockIt.PassOK = 1) Or (AppMode = "server") Then
